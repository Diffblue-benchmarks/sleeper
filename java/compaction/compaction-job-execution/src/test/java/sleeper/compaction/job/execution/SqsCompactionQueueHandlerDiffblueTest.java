...
class SqsCompactionQueueHandlerDiffblueTest {
   ...
   * <ul>
   *   <li>Given {@link InstanceProperties} {@link SleeperPropertyValues#getInt(SleeperProperty)} return one.</li>
   *   <li>Then calls {@link SleeperPropertyValues#getInt(SleeperProperty)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link SqsCompactionQueueHandler#receiveMessage()}
   */
  @Test
  @DisplayName("Test receiveMessage(); given InstanceProperties getInt(SleeperProperty) return one; then calls getInt(SleeperProperty)")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"Optional SqsCompactionQueueHandler.receiveMessage()"})
  void testReceiveMessage_givenInstancePropertiesGetIntReturnOne_thenCallsGetInt() throws IOException {
    try (MockedStatic<InetAddress> mockInetAddress = mockStatic(InetAddress.class)) {
      ...
      // Act
      Optional<MessageHandle> actualReceiveMessageResult = (new SqsCompactionQueueHandler(sqsClient,
          instanceProperties)).receiveMessage();
  ...
  void testReceiveMessage_thenReturnNotPresent() throws IOException {
    try (MockedStatic<InetAddress> mockInetAddress = mockStatic(InetAddress.class)) {
      ...
      // Act
      Optional<MessageHandle> actualReceiveMessageResult = (new SqsCompactionQueueHandler(sqsClient,
          new InstanceProperties())).receiveMessage();
