...
class DirectQueryDriverDiffblueTest {
  /**
   * Test {@link DirectQueryDriver#allTablesDriver(SystemTestInstanceContext, SystemTestClients)}.
   * <p>
   * Method under test: {@link DirectQueryDriver#allTablesDriver(SystemTestInstanceContext, SystemTestClients)}
   */
  @Test
  @DisplayName("Test allTablesDriver(SystemTestInstanceContext, SystemTestClients)")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({
      "sleeper.systemtest.dsl.query.QueryAllTablesDriver DirectQueryDriver.allTablesDriver(SystemTestInstanceContext, SystemTestClients)"})
  void testAllTablesDriver() {
    // Arrange, Act and Assert
    assertTrue(DirectQueryDriver.allTablesDriver(
        ...
        mock(SystemTestClients.class)) instanceof QueryAllTablesInParallelDriver);
  }

  /**
   * Test {@link DirectQueryDriver#run(Query)}.
   * <ul>
   *   <li>Given {@link IOException#IOException(String)} with {@code foo}.</li>
   *   <li>Then throw {@link UncheckedIOException}.</li>
   * </ul>
   * <p>
   * Method under test: {@link DirectQueryDriver#run(Query)}
   */
  @Test
  @DisplayName("Test run(Query); given IOException(String) with 'foo'; then throw UncheckedIOException")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"java.util.List DirectQueryDriver.run(Query)"})
  void testRun_givenIOExceptionWithFoo_thenThrowUncheckedIOException() {
    // Arrange
    ...
    SystemTestClients clients = sqsV2Result.sts(new AWSSecurityTokenServiceAsyncClient())
        ...
        .build();
    DirectQueryDriver directQueryDriver = new DirectQueryDriver(instance, clients);
    Query query = mock(Query.class);
    when(query.getTableName()).thenThrow(new UncheckedIOException(new IOException("foo")));

    // Act and Assert
    assertThrows(UncheckedIOException.class, () -> directQueryDriver.run(query));
