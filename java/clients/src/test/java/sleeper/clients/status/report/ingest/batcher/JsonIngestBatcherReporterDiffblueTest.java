...
class JsonIngestBatcherReporterDiffblueTest {
  /**
   * Test {@link JsonIngestBatcherReporter#report(List, Type, TableStatusProvider)}.
   * <ul>
   *   <li>Then calls {@link TableStatusProvider#getById(String)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link JsonIngestBatcherReporter#report(List, Type, TableStatusProvider)}
   */
  @Test
  @DisplayName("Test report(List, Type, TableStatusProvider); then calls getById(String)")
  @Tag("MaintainedByDiffblue")
  @MethodsUnderTest({"void JsonIngestBatcherReporter.report(List, Type, TableStatusProvider)"})
  void testReport_thenCallsGetById() {
    // Arrange
    JsonIngestBatcherReporter jsonIngestBatcherReporter = new JsonIngestBatcherReporter();

    ArrayList<FileIngestRequest> fileList = new ArrayList<>();
    ...
    // Act
    jsonIngestBatcherReporter.report(fileList, Type.PROMPT, tableProvider);
  ...
  void testReport_thenCallsGetTableByUniqueId() {
    ...
    // Act
    jsonIngestBatcherReporter.report(fileList, Type.PROMPT, new TableStatusProvider(tableIndex));
